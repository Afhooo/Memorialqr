...
export function MemorialNavbar({ memorialName, memoryCount, lastUpdatedLabel }: MemorialNavbarProps) {
  const items = [
    { href: "#hero", label: "Portada", icon: "🪄" },
    { href: "#intro", label: "Presentación", icon: "📜" },
    { href: "#muro", label: "Feed", icon: "💬" },
    { href: "#galeria", label: "Galería", icon: "🖼️" },
    { href: "#historia", label: "Línea de vida", icon: "⏳" },
    { href: "#compartir", label: "Compartir", icon: "🔗" },
  ];
  ...
  return (
    <nav className="relative flex h-fit flex-col gap-4 overflow-hidden rounded-[22px] border border-white/50 bg-white/85 px-4 py-4 text-[#0f172a] shadow-[0_16px_50px_rgba(0,0,0,0.14)] backdrop-blur-xl">
      ...
      <div className="relative flex items-center justify-between gap-3">
        <div>
          <p className="text-xs font-semibold text-[#e87422]">Story canvas</p>
          <p className="text-base font-semibold text-[#0f172a] leading-tight">Muro de {memorialName}</p>
